...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">请假信息管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active">请假信息列表</li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">请假信息列表</h3>							
												...
												<div class="custom-control custom-checkbox">
													...
													<label class="custom-control-label" for="select-all"></label>
																																						...
																																						<th onclick="sort('mingcheng')">名称<i id="mingchengIcon" class="fa fa-sort"></i></th>
																																																			<th onclick="sort('qingjiariqi')">请假日期<i id="qingjiariqiIcon" class="fa fa-sort"></i></th>
																																																			<th onclick="sort('qingjiatianshu')">请假天数<i id="qingjiatianshuIcon" class="fa fa-sort"></i></th>
																																																			<th onclick="sort('jieshuriqi')">结束日期<i id="jieshuriqiIcon" class="fa fa-sort"></i></th>
	...
	<script>
			...
			row.appendChild(checkbox)
							...
							var mingchengCell  = document.createElement('td');
									...
									row.appendChild(mingchengCell);
				            				var qingjiariqiCell  = document.createElement('td');

												  qingjiariqiCell.innerHTML = item.qingjiariqi;

				
									row.appendChild(qingjiariqiCell);
				            				var qingjiatianshuCell  = document.createElement('td');

												 qingjiatianshuCell.innerHTML = item.qingjiatianshu;

				
									row.appendChild(qingjiatianshuCell);
				            				var jieshuriqiCell  = document.createElement('td');

												  jieshuriqiCell.innerHTML = item.jieshuriqi;

				
									row.appendChild(jieshuriqiCell);
				            				var qingjianeirongCell  = document.createElement('td');

												  if(item.qingjianeirong != null && item.qingjianeirong != "" && item.qingjianeirong.length >= 11){
				  	qingjianeirongCell.innerHTML = item.qingjianeirong.substring(0, 10) + "..."; 
				  }else{
				  	qingjianeirongCell.innerHTML = item.qingjianeirong;
				  }

				
									row.appendChild(qingjianeirongCell);
		...
		function sh(){
			...
			httpJson("qingjiaxinxi/update","POST",data,(res)=>{
				if(res.code == 0){
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("qingjiaxinxi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function crossTable(id,crossTableName){
			...
			window.sessionStorage.setItem('crossTableName',"qingjiaxinxi");
